...
class MatchViewSet(viewsets.ModelViewSet):
    ...
    def pdf_gen (self, request, pk=None):
        ...
        response['Content-Disposition'] = f'attachment; filename="Protokoll {match.homeTeamId.name} vs {match.awayTeamId}"'
        ...
        def point_sum(player):
            ...
            points = 0
            points += (1 * match.event_set.filter(player1 = player, event_type="1P").count()) + (2 * match.event_set.filter(player1 = player, event_type="2P").count()) + (3 * match.event_set.filter(player1 = player, event_type="3P").count()) 
        ...
        def foul_sum(player):
            fouls = 0
            fouls += (match.event_set.filter(player1 = player, event_type="FO").count() + match.event_set.filter(player1 = player, event_type="FP").count())
            return str(fouls)
        ...
        doc.build([titleMain,titleMatchTime, spacer, titleHomeT, tableHomeT, spacer, titleAwayT, tableAwayT, spacer, titleEvents, tableEvent])
